...
def export_plotly(plotly_df,
    ...
    for system, df in plotly_df.groupby('system'):
        ...
        for state in state_columns:
            color_mappings[state] = df[df.traj_time%10000 == 0][state].values
        color_mappings['frame'] = df[df.traj_time%10000 == 0]['frame'].values
        color_mappings['traj_time'] = df[df.traj_time%10000 == 0]['traj_time'].values
    ...
    botton_lists = []
    button_lists.append(dict(label="Only FES",
                     method="restyle",
                     visible=True,
    ...
    for i, state in enumerate(state_columns):
        ...
        button_lists.append(dict(label=state,
                            method="restyle",
                            visible=True,
                            args=[{"marker.color": color_mappings[state]}],
                            ))

...
class MDDataFrame(object):
    ...
    def add_metadata(self, trajectory_prot_files, trajectory_sys_files, stride):
        """
        ...
        for sys, df in self.dataframe.groupby(['system']):
            print(f'{df.pathway.iloc[-1]} system {df.seed.iloc[-1]}: {df.traj_time.iloc[-1] / 1000} ns')


    def _create_dask_dataframe(self, npartitions=None):
        self.dd_dataframe = dd.from_pandas(self.dataframe, npartitions=npartitions)
...
class AnalysisResult(dict):
    ...
    def add_column_to_results(self, analysis_function, **kwargs):
        ...
        self[item_name] = self.md_data.map_partitions(lambda df:
                                        df.assign(**{item_name: analysis_function(**kwargs)(df)}),
                                        meta=meta,
                                        token=item_name
                                        ).persist()
        
        self.ref_info[item_name] = analysis_function(**kwargs).run_analysis(u_ref,0,3,1)
    ...
    def filter_result(self, column, filter_threshold):
        ...
        for datafile in set(self[column].iloc[:, 1]):
            ...
            filter_index.extend(np.where(np.any(data<filter_threshold, axis=0)))
        filter_index = list(set(filter_index[0]))

        for datafile in set(self[column].iloc[:, 1]):
            data = np.load(datafile, allow_pickle=True)
            np.save(datafile + '.filter_' + str(filter_threshold) + '.npy', data[: , filter_index])
            
        self[column + '.filter_' + str(filter_threshold)] = self[column].copy(deep=True)
        self[column + '.filter_' + str(filter_threshold)].iloc[:, 1] = self[column].iloc[:, 1].apply(lambda x: x + '.filter_' + str(filter_threshold) + '.npy')
...
class DaskChunkMdanalysis(object):
    ...
    def run_df(self, df):
        ...
        for system, df_sys in df.groupby(['system']):
            # if system information has to be used set `universe_file = 'system'`
...
class get_backbonetorsion(DaskChunkMdanalysis):
    ...
    def run_analysis(self, universe, start, stop, step):
        res_phi_list = [x.resindices[0] for x in universe.atoms.residues.phi_selections() if x is not None]

        res_psi_list = [x.resindices[0] for x in universe.atoms.residues.psi_selections() if x is not None]
        ...
        self._feature_info.extend(['_'.join([str(residex), ang]) for residex, ang in
                             itertools.product(res_psi_list,
                                               ['psi_cos', 'psi_sin'])])
        phi_ag = [x for x in universe.atoms.residues.phi_selections() if x is not None]
        ...
        torsion_angle = torsion_angle.reshape(torsion_angle.shape[0], torsion_angle.shape[1] * torsion_angle.shape[2])
...
class get_atomic_position(DaskChunkMdanalysis):
    ...
    def run_analysis(self, universe, start, stop, step):
        ...
        self._feature_info = ['_'.join([str(residex), feat]) for residex, feat in itertools.product(CA_atoms.residues.resindices, ['_ca_pos_x', '_ca_pos_y','_ca_pos_z'])]

        result = []
        for ts in universe.trajectory[start:stop:step]:
            result.append((CA_atoms.positions - CA_atoms.center_of_geometry()).ravel())
...
class get_domain_position(DaskChunkMdanalysis):
    ...
    def run_analysis(self, universe, start, stop, step):
        ...
        for domain_name, selection in domain_dict.items():
            for domain_subunit in universe.select_atoms(selection).split('segment'):
                domain_ag_list.append(domain_subunit)
                domain_info_list.append(domain_name + '_' + domain_subunit.segids[0] + '_pos')
                
        domain_ag_list = np.asarray(domain_ag_list, dtype=object).reshape(len(domain_dict),5).T.ravel()
        domain_info_list = np.asarray(domain_info_list, dtype=object).reshape(len(domain_dict),5).T.ravel()
...
class get_domain_interdistance(DaskChunkMdanalysis):
    ...
    def run_analysis(self, universe, start, stop, step):
        ...
        domain_ag_list = np.asarray(domain_ag_list, dtype=object).reshape(len(domain_dict),5).T
        domain_info_list = np.asarray(domain_info_list, dtype=object).reshape(len(domain_dict),5).T
        
        self._feature_info = []
        for feature_chain in [[0,1,2], [1,2,3], [2,3,4], [3,4,0], [4,0,1]]:
            for d1_inf, d2_inf in itertools.product(domain_info_list[feature_chain[0]], domain_info_list[feature_chain].ravel()):
                if d1_inf != d2_inf:
        ...
        for ts in universe.trajectory[start:stop:step]:
            r_ts = []
            for feature_chain in [[0,1,2], [1,2,3], [2,3,4], [3,4,0], [4,0,1]]:
                for domain_ag1, domain_ag2 in itertools.product(domain_ag_list[feature_chain[0]], domain_ag_list[feature_chain].ravel()):
...
class get_domain_intradistance(DaskChunkMdanalysis):
    ...
    def run_analysis(self, universe, start, stop, step):
        ...
        for feature_chain in range(5):
            for d1_inf, d2_inf in itertools.product(domain_info_list[feature_chain], domain_info_list[feature_chain].ravel()):
                if d1_inf != d2_inf:
                    self._feature_info.append('_'.join([d1_inf, d2_inf]))
        ...
        for ts in universe.trajectory[start:stop:step]:
            r_ts = []
            for feature_chain in range(5):
                for domain_ag1, domain_ag2 in itertools.product(domain_ag_list[feature_chain], domain_ag_list[feature_chain].ravel()):
                    if domain_ag1 != domain_ag2:
                        r_ts.append(1.0 / calc_bonds(domain_ag1.center_of_geometry(),
                                                        domain_ag2.center_of_geometry()))
            result.append(r_ts)
    ...
    def run_analysis(self, universe, start, stop, step):
        ...
        for ts in universe.trajectory[start:stop:step]:
            ...
            for selection1, selection2 in selection_comb:
                ...
                for sel in selection1.split(' '):
                    ag_sel1_collection.append(universe.select_atoms('name CA and chainid ' + sel))
                for sel in selection2.split(' '):
                    ag_sel2_collection.append(universe.select_atoms('name CA and chainid ' + sel))
        ...
        for selection1, selection2 in selection_comb:
            for feature_chain in feature_list:
                self._feature_info.append(selection1.split( )[0] + '_' + feature_chain)
    ...
    def run_analysis(self, universe, start, stop, step):
        ...
        for ts in universe.trajectory[start:stop:step]:
            ...
            for selection1, selection2 in selection_comb:
                ...
                filtered_distance_matrix = []
                for i,j in filtered_candidatess:
                    filtered_distance_matrix.append(1.0 / distance_matrix[i,j])
...
class get_rmsd_ref(DaskChunkMdanalysis):
    ...
    def run_analysis(self, universe, start, stop, step):
        ...
        name_ca = universe.select_atoms('name CA')
        rmsd_bgt = RMSD(name_ca, u_ref.select_atoms('name CA'), ref_frame=0).run(start, stop, step)
        rmsd_epj = RMSD(name_ca, u_ref.select_atoms('name CA'), ref_frame=1).run(start, stop, step)
        rmsd_pnu = RMSD(name_ca, u_ref.select_atoms('name CA'), ref_frame=2).run(start, stop, step)
...
class get_pore_hydration(DaskChunkMdanalysis):
    ...
    def run_analysis(self, universe, start, stop, step):
        ...
        pore_hydrat_n = universe.select_atoms("(cyzone 9 6 -6 resid 247) and resname TIP3 and name OH2",updating = True)

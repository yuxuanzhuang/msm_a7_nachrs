...
parser.add_argument('--starting_pdb', type=str, required=False, default='pdb2gmx.pdb')
parser.add_argument('--MembraneType', type=str, required=False, default="POPC:CHL1")
...
## Copy from https://github.com/pstansfeld/MemProtMD/blob/main/MemProtMD_insane.ipynb
...
visualization = False
name = args.name
starting_pdb = args.working_dir + '/../' + seed_loc + '/' + args.system + '/' + args.seed + '/' + args.starting_pdb

Box_Width = 13
Box_Length = 16.6
cube = [Box_Width, Box_Width, Box_Length]

working_dir = args.working_dir + '/../' + seed_loc + '/' + args.system + '/' + args.seed + '/' + name + '/'
...
# find TMD region
os.system('/nethome/yzhuang/git_repo/memembed/bin/memembed -q 1 -o ' + working_dir + '/memembed.pdb ' + starting_pdb) 
...
for file in glob.glob(r'./common_files/*'):
    if not os.path.isdir(file):
        ...
        shutil.copytree(file, working_dir + '/' + os.path.basename(file), dirs_exist_ok=True)
...
gromacs.editconf(f='memembed.pdb',o='centered.pdb', n='index.ndx',c=True,input=(0,0),box=cube,backup=False)
...
align.AlignTraj(u_ref, u, select="name BB", weights="mass", in_memory=True).run()
print(mda.analysis.rms.rmsd(u_ref.select_atoms('name BB').positions, u.select_atoms('name BB').positions))
...
os.system('python2 insane3.py ' + lipid + ' -salt 0.15 -sol W -o CG-system.gro -p topol.top -f protein-cg-chol.pdb -center -x %s -y %s -z %s -dm %s' % (x, y, z, dm))
...
mdrun_em = MDrunner_d(deffnm='em', cpi='em', c='CG-system.pdb',backup=False, ntmpi=1, ntomp=12)
mdrun_em.run()
...
with mda.selections.gromacs.SelectionWriter('index.ndx', mode='w') as ndx:
    ...
    ndx.write(u.select_atoms('not protein and not resname W ION'), name='Lipid')
...
gromacs.grompp(f='cgmd.mdp',o='MD/md',c='CG-system.pdb', r='CG-system.pdb', maxwarn=-1, n='index.ndx',backup=False)
...
gromacs.mdrun(deffnm='md', cpi='md', backup=False, nsteps=400000, ntmpi=1, ntomp=12, gpu_id=str(args.gpu_id))
...
memb_water = u.select_atoms(f'resname SOL and same residue as (resname SOL and prop z <= {com_popc_z + 19} and prop z >= {com_popc_z - 19})')
# memb_water
# memb_water.groupby('resnames')
u.select_atoms('not group memb_water', memb_water=memb_water).write(working_dir + 'FINAL/' + name + '-prot_memb.pdb')
...
for file in glob.glob(working_dir + r'/at/*'):
    if not os.path.isdir(file):
        ...
        shutil.copytree(file, working_dir + 'FINAL/charmm36.ff', dirs_exist_ok=True)
...
align.AlignTraj(u_aligned, u, select="name CA", weights="mass", in_memory=True).run()
print(mda.analysis.rms.rmsd(u_aligned.select_atoms('name CA').positions, u.select_atoms('name CA').positions))
u_merge = mda.core.universe.Merge(u_aligned.atoms, u.select_atoms('not protein'))
...
gromacs.grompp(f='em.mdp',o='em_test.tpr', p='topol_test.top', c=name + '-test.pdb', r=name + '-test.pdb',maxwarn='-1',backup=False,v=True)
...
if n_charge > 0:
    ...
    u.select_atoms('not group remove_sod', remove_sod=remove_sod).write(name + '-final.pdb')
    mol_dict['NA'] = str(eval(mol_dict['NA']) - n_charge)
elif n_charge < 0:
    remove_cla = u.select_atoms('resname CL')[:abs(n_charge)]
    u.select_atoms('not group remove_cla', remove_cla=remove_cla).write(name + '-final.pdb')
...
gromacs.grompp(f='em.mdp',o='em.tpr', p='topol.top', c=name + '-final.pdb', r=name + '-final.pdb',maxwarn='-1',backup=False,v=True)
...
mdrun_em_aa = MDrunner_d(deffnm='em', cpi='em', c='start.pdb', backup=False, ntmpi=1, ntomp=12)
mdrun_em_aa.run()
...
with mda.selections.gromacs.SelectionWriter('index.ndx', mode='w') as ndx:
    ...
    ndx.write(u.select_atoms('not protein and not resname SOD CLA TIP3'), name='MEMB')

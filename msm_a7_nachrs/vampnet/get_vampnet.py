...
try:
    ...
    data_load = np.load(pd.read_pickle(
        'msm_features_new.pickle')[feature_name][0])
    data_shape = data_load[:, vampnet_obj.feed_feature_indice].shape[1]
pentamer_lobe = torch.nn.DataParallel(
    MultimerNet(
...
for i, (vampnet, n_epoch) in enumerate(zip(vampnets, n_epochs)):
    vampnet.fit(loader_train,
                ...
                progress=tqdm)
    torch.save(vampnet.lobe.module.state_dict(),
               f'./test_lobe_nstates_{n_states}_epoch_{n_epoch}_{i}.lobe')
    pickle.dump(
        vampnet,
        open(
            f'./vampnet_nstates_{n_states}_epoch_{n_epoch}_{i}.pickle',
            'wb'))
    print(i)

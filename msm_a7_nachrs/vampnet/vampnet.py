...
class VAMPNet_Multimer_Rev_Model(VAMPNet_Multimer, Transformer):
    def __init__(self,
                ...
                dtype=np.float32):
        super().__init__(multimer, n_states, lobe, lobe_timelagged, device, optimizer, learning_rate, score_method, score_mode, epsilon, dtype)
        self._ulayer = ulayer
        self._slayer = slayer
...
class VAMPNet_Multimer_Rev(VAMPNet_Multimer, Transformer, DLEstimatorMixin):
    ...
    def fit(self, data_loader: torch.utils.data.DataLoader, n_epochs=1, validation_loader=None,
        ...
        **kwargs
        ...
        with disable_TF32():
            ...
            if train_mode=='all':
                print("Train all")
                for epoch in progress(range(n_epochs), desc="Train all VAMPNet epoch", total=n_epochs, leave=False):
                ...
                if train_mode=='us' or train_mode=='u':
                    ...
                    for epoch in progress(range(n_epochs), desc="Train u/s VAMPNet epoch", total=n_epochs, leave=False):
                        self.optimizer_u.zero_grad()
                        if train_mode=='us':
                            self.optimizer_s.zero_grad()
                ...
                if train_mode=='s':
                    ...
                    for epoch in progress(range(n_epochs), desc="Train s VAMPNet epoch", total=n_epochs, leave=False):
                        self.optimizer_s.zero_grad()
    ...
    def fit_routine(self, data_loader: torch.utils.data.DataLoader, n_epochs=1, validation_loader=None,
            ...
            train_score_callback: Callable[[int, torch.Tensor], None] = None,
            validation_score_callback: Callable[[int, torch.Tensor], None] = None, tb_writer=None,
        ...
        **kwargs
        ...
        with disable_TF32():
            ...
            for g in self.optimizer_s.param_groups:
                lr_s = g['lr']                
            for epoch in progress(range(n_epochs), desc="Train only VAMPNet epoch", total=n_epochs, leave=False):
                for g in self.optimizer_u.param_groups:
                    g['lr'] = lr_u/10
                for g in self.optimizer_s.param_groups:
                    g['lr'] = lr_s/10
        ...
        **kwargs
        ...
        with disable_TF32():
            ...
            if train_mode=='all':
                ...
                for epoch in progress(range(n_epochs), desc="Train single VAMPNet epoch", total=n_epochs, leave=False):

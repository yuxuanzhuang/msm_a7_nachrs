...
def start_training(
    ...
    for i, vampnet in enumerate(vampnets):
        ...
        if os.path.isfile(f'{msm_obj.filename}{class_name}/epoch_{n_epochs}_state_{vampnet.n_states}_rep_{rep}.lobe') and not overwrite:
            vampnets[i] = pickle.load(open(f'{msm_obj.filename}{class_name}/epoch_{n_epochs}_state_{vampnet.n_states}_rep_{rep}.lobe',
                                           'rb'))
            continue

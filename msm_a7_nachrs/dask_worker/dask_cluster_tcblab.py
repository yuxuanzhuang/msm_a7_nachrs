...
try:
    ni.ifaddresses('ens8')
    ip_address = ni.ifaddresses('ens8')[ni.AF_INET][0]['addr']
except BaseException:
    pass
try:
    ni.ifaddresses('enp195s0')
...
#        self.jobscript.append("#SBATCH -p lindahl\n")
        ...
        self.jobscript.append("#SBATCH -t 48:00:00\n")
...
class LindahlSLURMJob(SLURMJob):
    def _generate_job(self):
        self.jobscript = []
        self.jobscript.append("#!/usr/bin/env bash \n")
        self.jobscript.append("#SBATCH -J dask-worker\n")
#        self.jobscript.append("#SBATCH -p lindahl\n")
        ...
        self.jobscript.append("#SBATCH --cpus-per-task=1\n")
        self.jobscript.append("#SBATCH --ntasks-per-core=1\n")
        self.jobscript.append("#SBATCH --ntasks-per-node=24\n")
        self.jobscript.append("#SBATCH -t 48:00:00\n")
    ...
    def submit_job(self):
        with open('/nethome/yzhuang/dask-worker-space-lindahl/submit.sh', "w") as f:
            f.writelines(self.jobscript)
        proc = subprocess.Popen(
            ['sbatch', 'submit.sh'], stdout=subprocess.PIPE, stderr=subprocess.PIPE,
            cwd='/nethome/yzhuang/dask-worker-space-lindahl'
        )
...
class NewLindahlSLURMJob(SLURMJob):
    def _generate_job(self):
        self.jobscript = []
        self.jobscript.append("#!/usr/bin/env bash \n")
        self.jobscript.append("#SBATCH -J dask-worker\n")
#        self.jobscript.append("#SBATCH -p lindahl\n")
        self.jobscript.append("#SBATCH -p lindahl1,lindahl2,lindahl3,lindahl4\n")
        ...
        self.jobscript.append("#SBATCH --ntasks-per-node=24\n")
        self.jobscript.append("#SBATCH -t 48:00:00\n")
        self.jobscript.append("#SBATCH --gres=gpu:4\n")
        self.jobscript.append("\n")
        self.jobscript.append(
            "srun " +
            dask_worker_dir +
    ...
    def submit_job(self):
        ...
        proc = subprocess.Popen(
            [
            '/opt/tcbsys/slurm/22.05.3/bin/sbatch',
            'submit.sh'], stdout=subprocess.PIPE, stderr=subprocess.PIPE,
            cwd='/nethome/yzhuang/dask-worker-space-lindahl'
        )
